...
def test_get_image_not_found(client):
    resp = client.get("/images/ALPHA_20240115T120000_999?instrument=ALPHA")
    assert resp.status_code == 404


def test_ingest_and_get_image(client):
...
def test_query_images_time_range(client):
...
def test_query_images_missing_params(client):
    resp = client.get("/images", params={"instrument": "ALPHA"})
